...
async function main(){
        ...
        await tab.click('button[class="login-cta"]', {delay: 50})
        ...
        let allInternship = await tab.$$('.container-fluid.individual_internship.view_detail_button.visibilityTrackerItem ', {delay : 50});
        console.log('Total question' , allInternship.length);

        let xPath = await getXPathFromHandle(allInternship[0]); // Get the XPath
        ...
        // await tab.waitForSelector('.buttons_container a > button[class="btn btn-large"]', { timeout: 30000 });

        
        // Click the button
        // await tab.click('#apply_now_button');
        //await selectedInternship(allInternship);
        //await tab.click('.internship_listing_card a');
        ...
        // await tab.waitForSelector(".browse_container.no_applications a.btn.btn-primary", {visible : true});
        //await tab.$$("i.ic-16-chevron-right");
...
async function getXPathFromHandle(allInternship) {
    return await allInternship.click((element) => {
        let path = '';
